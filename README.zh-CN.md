...
- 每个 Windows 登录会话只允许一个应用实例运行；重复启动时会恢复已有窗口。
...
- 在界面或 SSH 初始化前使用 Windows 命名互斥体阻止重复应用实例；命名自动复位事件把重复启动通知转发给第一进程，再由第一进程在 GPUI 线程恢复并激活已有窗口。
...
- Windows 进程协调与窗口可见性代码统一隔离在条件编译的平台模块中，未来增加 macOS 与 Linux 后端时不会把原生 API 泄漏进 GPUI 应用层。
